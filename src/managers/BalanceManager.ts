  ...
  implements NetworkUpdateListener
{
  ...
  protected onTokenBalanceUpdate(
    networkName: string,
    symbol: string,
    newBalance: bigint
  ): void {
  ...
  protected async updateTokenBalances(
  ...
  public hasTokenBalance(
    ...
    const balance = this.balances.get(networkName);
    return balance
      ? (balance.tokens.get(symbol) || 0n) > minimumBalance
      : false;
  }
  ...
  protected async updateNativeBalances(
    networks: ConceroNetwork[]
  ): Promise<void> {
  ...
  public async ensureAllowance(
    ...
    if (currentAllowance >= requiredAmount) {
      const effectiveMinAmount =
        minAmount >= requiredAmount ? minAmount : requiredAmount;
    ...
    const newAllowance =
      requiredAmount > minAmount ? requiredAmount : minAmount;
    this.logger.info(
